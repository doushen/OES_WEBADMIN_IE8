...
var Layout = function () {
    ...
    var handleSidebarAndContentHeight = function () {
        ...
        if (body.hasClass("page-footer-fixed") === true && body.hasClass("page-sidebar-fixed") === false) {
            ...
            $(".tab-pane iframe").height(getIframeHeight());
        }
    };
    ...
    var handleSidebarMenu = function () {
        ...
        // handle link iframe tab
        $('.page-sidebar,.dropdown-menu,.head-notice').on('click', ' li > a.iframeify', function (e) {
            e.preventDefault();
            App.scrollTop();
            // var closeStr = $(this).attr("close");
            // if(!closeStr){
            //     closeStr = 'true';
            // }
            // var enableClose = closeStr.toLowerCase() === 'true';

            // addTabs({id: $(this).attr("id"), title: $(this).attr('title'), url:$(this).attr("href"), close: enableClose});
    ...
    var getIframeHeight = function () {
        return parseInt($('.page-content').css('min-height'))-53-6;
    };
    ...
    var addTabs = function (obj) {
        ...
        if (!$("#" + id)[0]) {
            ...
            //加入TABS
            if ($("#tab_tab_000-000").length > 0) {
                $("#tab_tab_000-000").after(title);
                $("#tab_000-000").after(content);
            }
            else {
    ...
    var closeTab = function (id) {
        ...
        if ($(".nav-tabs>li.active[role=presentation]").attr('id') == "tab_" + id) {
                $("#tab_" + id).prev().addClass('active');
                $("#" + id).prev().addClass('active');
        }else if ($(".nav-tabs>li>.dropdown-menu>li.active").attr('id') == "tab_"+id){
            if($("#tab_" + id).prev().length>0){
                // $("#tab_" + id).prev().addClass('active');
             //    $("#" + id).prev().addClass('active');
                var tid = $("#tab_" + id).prev().attr('id');
                tid = tid.split('_')[tid.split('_').length-1];
                tid = 'tab_'+tid;
                $("#tab_" + tid).addClass('active');
                $("#" + tid).addClass('active');
            }else{
                var tid = $(".nav-tabs>li").last().attr('id');
                tid = tid.split('_')[tid.split('_').length-1];
                tid = 'tab_'+tid;
                $("#tab_" + tid).addClass('active');
                $("#" + tid).addClass('active');
    ...
    // Handles FullScreen toggler
    var handleFullscreenToggler = function() {

        // var fullScreen = function () {
        //     //进入全屏
        //     var requestFullScreen = function () {
        //         // var de = document.documentElement;
        //         // var tbid = $(".tab-pane.active").attr("id");
        //         // var de = document.getElementById(tbid).firstChild;
        ...
        //         var de = document.getElementById($(".tab-pane.active").attr("id")).firstChild;;
        //         if (de.requestFullscreen) {
        //             de.requestFullscreen();
        //         } else if (de.mozRequestFullScreen) {
        //             de.mozRequestFullScreen();
    ...
    return {
        ...
        initSidebar: function($state) {
            ...
            $(".page-sidebar .nav-item.active>a").click();
        },
        ...
        initFooter: function() {
            ...
            handleFullscreenToggler(); //handles fullscreen toggler
        },
        ...
        addTab: function(obj) {
            var closeStr = $(obj).attr("close");
            if(!closeStr){
                closeStr = 'true';
            }
            var enableClose = closeStr.toLowerCase() === 'true';

            addTabs({id: $(obj).attr("id"), title: $(obj).attr('title'), url:$(obj).attr("href"), close: enableClose});
        },
